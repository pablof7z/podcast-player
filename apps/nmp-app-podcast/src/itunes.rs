//! iTunes Search API helpers used by `podcast.search_itunes` dispatch.
//!
//! Extracted from `host_op_handler.rs` to keep that file under the 500-line
//! hard limit. Logic is unchanged from the prior inline copy: a tiny
//! query-string URL encoder + the iTunes-response deserializer.

use crate::ffi::projections::PodcastSummary;

/// Percent-encode a query string for inclusion in an iTunes search URL.
///
/// Mirrors the legacy inline helper: space → `+`, unreserved alnum + `-._~`
/// pass through, everything else is `%XX` upper-case hex of the UTF-8 bytes.
