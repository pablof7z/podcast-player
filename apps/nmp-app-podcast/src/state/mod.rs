...
pub struct PodcastAppState {
    ...
    // In-app feedback runtime DROPPED in Epic A A0/A1: `nmp-feedback` has no
    // rev that pins past the nmp-ffi deletion, so the dependency is removed and
    // feedback re-integration is deferred (nmp-feedback#3 / podcast-player#597).
    // The FetchFeedback action + feedback snapshot slots return empty until the
    // re-integration slice lands.
