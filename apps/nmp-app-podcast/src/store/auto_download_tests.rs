...
/// Helper: call with wifi_only=false, is_on_wifi=true (the previous
/// default — all existing tests that didn't care about Wi-Fi gating).
fn auto_dl(
    fresh: &[Episode],
    ...
    episodes_to_auto_download(fresh, existing, local, auto_on, false, true)
}
...
fn auto_download_off_does_not_queue_download() {
    ...
    let out = auto_dl(&fresh, &HashSet::new(), &HashMap::new(), false);
    assert!(out.is_empty());
}
...
fn auto_download_on_queues_new_episodes() {
    ...
    let out = auto_dl(&fresh, &existing, &HashMap::new(), true);
    assert_eq!(out.len(), 1);
...
fn wifi_only_on_wifi_queues_episodes() {
    let pid = PodcastId::generate();
    let ep = make_episode(pid, "new", "https://ex.com/new.mp3");
    // wifi_only=true, is_on_wifi=true → should queue
    let out = episodes_to_auto_download(
        &[ep.clone()], &HashSet::new(), &HashMap::new(),
        true, true, true,
    );
    assert_eq!(out.len(), 1);
}
...
#[test]
fn wifi_only_on_cellular_skips_episodes() {
    let pid = PodcastId::generate();
    let ep = make_episode(pid, "new", "https://ex.com/new.mp3");
    // wifi_only=true, is_on_wifi=false → should NOT queue
    let out = episodes_to_auto_download(
        &[ep.clone()], &HashSet::new(), &HashMap::new(),
        true, true, false,
    );
    assert!(out.is_empty(), "wifi-only must not queue on cellular");
}
...
fn cellular_allowed_queues_on_cellular() {
    let pid = PodcastId::generate();
    let ep = make_episode(pid, "new", "https://ex.com/new.mp3");
    // wifi_only=false, is_on_wifi=false → cellular-ok, should queue
    let out = episodes_to_auto_download(
        &[ep.clone()], &HashSet::new(), &HashMap::new(),
        true, false, false,
    );
    assert_eq!(out.len(), 1, "cellular-allowed must queue even without Wi-Fi");
}
...
fn auto_download_off_with_wifi_still_returns_empty() {
    let pid = PodcastId::generate();
    let ep = make_episode(pid, "new", "https://ex.com/new.mp3");
    // auto_download_on=false wins even if wifi conditions are met
    let out = episodes_to_auto_download(
        &[ep], &HashSet::new(), &HashMap::new(),
        false, true, true,
    );
    assert!(out.is_empty());
}
