...
impl FollowListObserver {
    ...
    /// * `latest_kind3` — the store-backed latest-kind:3 reader
    ///   (`LatestKind3FollowSet::new(app.event_store_handle())`);
    ///   `FollowListProjection` is a thin read-model over the canonical event
    ///   store — no secondary map.
...
impl ObservedProjectionSink for FollowListObserver {
    ...
    fn on_kernel_event(&self, event: &KernelEvent) {
        ...
        // Read the follow list live from the canonical event store via the
        // inner FollowListProjection (a thin read-model — no secondary map).
        // The event store row is already durable by the time this sink fires,
        // so the re-read reflects this very event.
