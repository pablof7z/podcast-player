//! `nmp_app_podcast_openrouter_whisper_transcribe` — shared OpenRouter STT.
...
use crate::llm::openrouter_whisper::{self, OpenRouterWhisperError, OpenRouterWhisperIntent};

#[no_mangle]
#[allow(clippy::not_unsafe_ptr_arg_deref)]
pub extern "C" fn nmp_app_podcast_openrouter_whisper_transcribe(
    ...
    ffi_guard(
        ...
        || {
            ...
            let intent: OpenRouterWhisperIntent = match serde_json::from_str(json_str) {
                Ok(intent) => intent,
                Err(e) => {
            ...
            match runtime.block_on(openrouter_whisper::transcribe_openrouter_whisper(
                ...
                Err(error) => whisper_error_envelope(&error).into_raw(),
            }
        },
    )
}
...
fn whisper_error_envelope(error: &OpenRouterWhisperError) -> CString {
