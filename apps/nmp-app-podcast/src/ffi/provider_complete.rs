//! `nmp_app_podcast_provider_complete` — shared provider one-turn completion.
...
pub extern "C" fn nmp_app_podcast_provider_complete(
    ...
    let intent: CompletionIntent = match serde_json::from_str(json_str) {
        Ok(intent) => intent,
    ...
    match runtime.block_on(provider_transport::complete(store, intent)) {
