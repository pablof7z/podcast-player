...
pub extern "C" fn nmp_app_podcast_set_data_dir(handle: *mut PodcastHandle, path: *const c_char) {
    ...
    ffi_guard(
        ...
        || {
            ...
            // `register` and before `PodcastApp.start` (see the caller contract above),
            ...
            // with; the actor thread is joined before `PodcastApp` drops, so the pointer
            // is valid here (this runs before `PodcastApp.start`, well before teardown).
            ...
            let outbound_loaded = {
                ...
                if non_empty {
                    handle
                        .state
                        .social
                        .seed_outbound_turns(restored.turns().to_vec());
                }
