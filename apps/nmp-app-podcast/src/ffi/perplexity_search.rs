//! `nmp_app_podcast_perplexity_search` — shared online-search transport.
...
pub extern "C" fn nmp_app_podcast_perplexity_search(
    ...
    let intent: PerplexitySearchIntent = match serde_json::from_str(json_str) {
        Ok(intent) => intent,
        Err(e) => {
...
fn search_error_envelope(error: &PerplexitySearchError) -> CString {
