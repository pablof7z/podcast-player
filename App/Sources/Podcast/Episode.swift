...
struct Episode: Codable, Sendable, Identifiable, Hashable {
    ...
    /// Ad segments detected by the Rust kernel's `podcast.chapters.compile`
    /// action from the transcript. `nil` when detection hasn't been run yet
    /// (or the transcript wasn't ready); an empty array means detection ran
    /// and found no ads. Drives the auto-skip wiring in `PlaybackState` and
    /// the amber stripe on the chapter rail. Older saved state decodes via
    /// `decodeIfPresent` so the migration is silent.
...
extension Episode {
    ...
    struct Chapter: Codable, Sendable, Hashable, Identifiable {
        ...
        /// `true` when the chapter was synthesised by the kernel's
        /// `podcast.chapters.compile` action rather than parsed from a publisher
        /// source. Defaults to `false`; the player surfaces a subtle "AI" tag
        /// for true.
        var isAIGenerated: Bool
        /// 1–2 sentence summary of the chapter, generated by the kernel's
        /// chapter compile action alongside boundaries (FULL mode) or as an
        /// enrichment pass on publisher chapters (ENRICH-ONLY mode). `nil` until
        /// enrichment runs.
        var summary: String?
