...
protocol AgentOwnedPodcastManagerProtocol: Sendable {
    /// Create a new agent-owned synthetic podcast. A per-podcast Nostr pubkey
    ...
    /// publish NIP-F4 kind:10154 (show) + kind:54 (episode) events signed
    /// by the podcast key. No-ops when nostr is disabled or the parent podcast is
    /// not agent-owned / is private.
    /// Returns the published episode event id, or `nil` when
...
struct AgentOwnedPodcastInfo: Sendable {
    ...
    /// NIP-19 `npub` bech32 string for the podcast identity, if available.
