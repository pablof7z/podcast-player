...
actor LocalLLMService {
    ...
    nonisolated func infer(promptJSON: String) async -> String {
...
private func localLLMCallback(
    ...
    // The semaphore enforces a happens-before barrier: the box is written inside
    // the task before `signal()` and read here only after `wait()`, so the
    // single mutable field is never accessed concurrently.
    let box = InferenceResultBox()
...
// MARK: - Inference Result Box

/// Carries the inference result out of the detached `Task` and back to the
/// blocking C-callback thread. Safe to mark `@unchecked Sendable` because the
/// `DispatchSemaphore` in `localLLMCallback` enforces a happens-before barrier:
/// the field is written before `signal()` and read only after `wait()`.
private final class InferenceResultBox: @unchecked Sendable {
    var value: String = #"{"error":"Inference failed"}"#
}
