...
PodcastStore maintains a global is_on_wifi: bool flag set via the nmp.network.capability report. This is distinct from per-podcast wifi settings — it reflects the device's current network state. The episodes_to_auto_download function uses is_on_wifi combined with per-podcast wifi_only_for to determine whether to auto-download new episodes from a feed refresh. [^14943-68]
