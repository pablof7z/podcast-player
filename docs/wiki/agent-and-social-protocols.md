...
NIP-F4 is the canonical production protocol, not a legacy correction from NIP-74. It is the right NIP to use and produce. Agent-generated podcasts publish as NIP-F4 (kind:10154 for show + kind:54 for episode), not NIP-74 (kind:30074/30075). NIP-F4 canonical podcast publishing uses a per-podcast secp256k1 keypair, Blossom audio upload, and relay dispatch. Agent-generated podcasts intentionally do not auto-subscribe the user; they create a Podcast row but no PodcastSubscription, so episodes appear in 'See All' but not in the user's followed-podcasts list. No legacy NIP-74 data migration is needed because NMP is the only implementation. The podcast player's agent-owned podcast publishing path (`docs/features.md`) uses NIP-F4. NIP-74 references survive only as code-symbol names (e.g., `NIP74Show`, `NIP74Episode`) and anti-re-entry test guards that confirm NIP-74 tags are not emitted — these are work items that remove NIP-74, never move toward it.

<!-- citations: [^14943-3] [^14943-93] [^14943-98] [^e1cfd-1] -->
