...
The validation sequence for the integrated M1 stack is: (1) Rust cargo test -p nmp-app-podcast --lib — must pass all D5 contract tests (702+); (2) iOS app build via Xcode against the rebuilt sim lib; (3) live smoke test — library/inbox/picks render reactively, playback works (tap hero pick → mini-player tracks position, picks count updates via push); (4) focused Swift tests; (5) codex exec review --base main. After the Rust iOS sim lib was rebuilt and the stale local lib shadowing the fresh build was removed, the integration built clean and passed live verification. [^14943-19]
