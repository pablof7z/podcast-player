...
Scenario slugs must be stable. The generator uses the canonical scenario ID
lowercased, for example `fnd-001`, so title edits do not churn public URLs.
...
- Regeneration over an existing Pages checkout preserves `assets/` before
  ...
  disappear during scaffold refreshes.
- Existing per-scenario `data.json` records with the same schema are carried
  forward when they contain observed evidence, metrics, issues, non-`not_run`
  execution status, or a non-`incomplete` verdict. Fresh catalog/page metadata
  still wins for source and skill-grounding artifacts.
