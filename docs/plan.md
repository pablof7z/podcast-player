...
| Pod0 public validation command center: scenarios, screenshots, metrics, cassettes, UX critique, and NMP/chirp sync. | Active | `docs/plan/pod0-validation-command-center.md` |
...
- `docs/plan/pod0-validation-command-center.md` - public report publication, scenario evidence contract, issue loop, cassettes, metrics, UX critique, and NMP/chirp sync tracking.
...
| Area | Status |
|---|---|
| NMP dependency pin | Pinned to NMP master snapshot rev `a543943146aac1d98f657911ed682b9d974a1da4` (nmp-v0.8.4) in `Cargo.toml` as ordinary git deps — provisional until pablof7z/nostr-multi-platform#2690 cuts the v1 release-train tag (see `docs/plan/issue-597.md`, Epic A #597). `nmp-ffi`/`nmp-signer-broker`/`nmp-defaults` are deleted upstream and intentionally absent; the active integration branch compiles `apps/nmp-app-podcast` and `apps/podcast-tui` through `nmp-native-runtime`/`nmp-uniffi-support`/`nmp-substrate`. The native app-facing generic string bridge and app-owned C header are gone; explicit generated UniFFI methods now cover iOS, Android, TUI, and headless. Some Rust helper modules still use JSON-shaped internals behind that facade and remain cleanup tail, not app-facing API. `nmp-feedback` is still dropped pending pablof7z/nmp-feedback#3. Focused local validation shows a coherent NMP graph at this pin; full required merge-gate validation still belongs to CI. |
