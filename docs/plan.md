...
| Area | Status |
|---|---|
| NMP dependency pin | Pinned to NMP `origin/master` tip rev `a543943146aac1d98f657911ed682b9d974a1da4` (nmp-v0.8.4) in `Cargo.toml` as ordinary git deps — provisional until pablof7z/nostr-multi-platform#2690 cuts the v1 release-train tag (see `docs/plan/issue-597.md`, Epic A #597). `nmp-ffi`/`nmp-signer-broker`/`nmp-defaults` are deleted upstream and intentionally absent; the active integration branch compiles `apps/nmp-app-podcast` and `apps/podcast-tui` through `nmp-native-runtime`/`nmp-uniffi-support`/`nmp-substrate`. The native app-facing generic string bridge has been replaced by explicit generated UniFFI methods across iOS, Android, TUI, and headless; the remaining old Rust C-ABI endpoint modules are now only an internal implementation tail behind that facade. `nmp-feedback` is still dropped pending pablof7z/nmp-feedback#3. Focused local validation shows a coherent NMP graph at this pin; full required merge-gate validation still belongs to CI. |
